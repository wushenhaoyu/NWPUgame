...
import { _decorator, Component, Node, Label, SystemEvent, systemEvent, Prefab, instantiate, RigidBody2D, Vec2, CircleCollider2D, Contact2DType, Collider2D, IPhysics2DContact, BoxCollider2D, CCInteger, SpriteFrame, Sprite, UITransform, tween, Vec3, AudioClip, AudioSource, director, EventTouch, UITransformComponent, v3  } from 'cc';
...
export class GameManager extends Component {
    ...
    startMoveFruit (e:EventTouch) {
        if (this._currentFruit && this._FruitStatus === FruitStatus.INIT ) {
            ...
            tween(this._currentFruit)
                .to(0.1, { position: new Vec3(pos.x, this._currentFruit.position.y, 0) }, { onComplete: () => {
                    console.log(pos)
    ...
    //     if (this._currentFruit && this._FruitStatus === FruitStatus.INIT ) {
    //         tween(this._currentFruit)
    //             .to(0.1, { worldPosition: new Vec3(e.getLocationX(), this._currentFruit.getWorldPosition(new Vec3()).y, 0) }, { onComplete: () => {
    //             }, onUpdate: () => {
    //                 this._currentFruit.getChildByName('img').position = new Vec3(0,0,0)
    //             } })
    //             .start()
    ...
    //         const radius = this._currentFruit.getChildByName('img').getComponent(CircleCollider2D).radius
    //         x = x < radius ? radius : x
    //         x = x > (1000 - radius) ? (1000 - radius) : x
    //         this._currentFruit.setWorldPosition(new Vec3(x, this._currentFruit.getWorldPosition(new Vec3()).y, 0))
    //         this._currentFruit.getChildByName('img').position = new Vec3(0,0,0)
    //     }
    // }
    ...
    win()
    {
        ...
        this._GameStatus = GameState.SUCCESS
        // this._ScoreValue = 0
    ...
    lose()
    {
        this._GameStatus = GameState.FAIL
        // this._ScoreValue = 0
